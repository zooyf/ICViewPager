...
#define kDefaultTabOffsetFromCenter 0.0 // Offset of tabViewContent.center from tabView.center
#define kDefaultTabWidth 40
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.selected) {
        ...
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake(rect.size.width * 0.2, rect.size.height - 8.0)];
        [bezierPath addLineToPoint:CGPointMake(rect.size.width * 0.8, rect.size.height - 8.0)];
        [bezierPath setLineWidth:2.5];
...
- (void)reloadData {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        ...
        _tabOffsetFromCenter = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabOffsetFromCenter withDefault:kDefaultTabOffsetFromCenter];
    ...
    for (int i = 0; i < _tabCount; i++) {
        ...
        CGRect frame = tabView.frame;
        frame.origin.x = contentSizeWidth+kDefaultTabWidth/4.0;
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[_tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        CGPoint center = CGPointMake(tabView.center.x, tabView.center.y+self.tabOffsetFromCenter);
        tabViewContent.center = center;
